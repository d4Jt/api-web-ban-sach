...
const webhook_util_1 = __importDefault(require("../utils/webhook.util"));
const sync_util_1 = __importDefault(require("../utils/sync.util"));
const get_user_info_util_1 = __importDefault(require("../utils/get_user_info.util"));
...
class WebhookController {
    handlerBankTransfer = async (req, res) => {
        try {
            ...
            for (let item of req.body.data) {
                ...
                let orderId = webhook_util_1.default.parseOrderId(case_insensitive, transaction_prefix, item.description);
                console.log(orderId);
                ...
                if ((new Date().getTime() - new Date(item.when).getTime()) /
                    86400000 >=
                    expiration_date)
                    continue;
                ...
                orderData = orders.find((order) => {
                    if (order.captcha.substring(5) === orderId &&
                ...
                if (!orderData)
                    continue;
                await Order_model_1.default.updateOne({ _id: orderData._id }, { status: 'paid' });
            }
            return res.status(200).json({
                code: 200,
                message: 'success',
    ...
    usersPaid = async (req, res) => {
        try {
            ...
            await sync_util_1.default.syncTransaction(req.body.accountNumber);
            return res.status(200).json({
                code: 200,
                message: 'success',
                data: null,
    ...
    registerWebhook = async (req, res) => {
        try {
            ...
            await webhook_util_1.default.deleteWebhookByUrl(`${process.env.DOMAIN}/api/webhook/handler-bank-transfer`);
            ...
            let newWebhook = await webhook_util_1.default.create(data);
            // Lấy thông tin về userInfo
            let userInfo = await get_user_info_util_1.default.getDetailUser();
            return res.status(200).json({
                code: 200,
                message: 'success',
                data: {
    ...
    checkTransfer = async (req, res) => {
        try {
            ...
            const order = await Order_model_1.default.findOne({ captcha });
            if (!order) {
                return res.status(404).json({
                    code: 404,
